...
> 2026-09-05 补充：长期方向以[路线图](../../project/long-term-roadmap.md)衔接完整 MVP 及持续演进。本总览的 14 项按**工作包**管理；[交付规范](../worker-delivery-protocol.md)要求先完成任务卡再分派，详见 [45 项工作队列](2026-09-05-work-item-sequence.md)。首个工作包已有[四张完整任务卡](2026-09-05-p0-01-worker-packet.md)，不把远期不确定契约交给 worker 自行补齐。
...
每份子计划按工作包给出文件、接口、失败用例、实施步骤和提交边界。示例测试定义关键断言；同项列出的负向用例必须补齐，不能只运行示例就宣称验收完成。分派时以经过契约核验的完整任务卡为单位，技术负责人承担设计未知和门禁结论，worker 承担白名单内实现与证据采集。
