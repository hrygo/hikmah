...
- 本批准记录用于追溯，不替代正式规范。
- 最终技术要求以 `docs/product/overview.md`、目标版本基线和 Accepted ADR（ADR-0001、ADR-0003～ADR-0007）为准。
...
## 2026-09-05 知识协作试点与运行边界优化

用户选择首批试用价值为“团队知识问答与方案协作”，随后对“按 A 更新 PRD、相关 ADR 和审查跟踪表，先只优化文档”明确回复“同意”。此批准与 2026-08-28 的同名文档归一化方案分别记录。

- 接受 [ADR-0007](../decisions/0007-knowledge-collaboration-pilot-and-runtime-boundaries.md)：完整 MVP 终态保留，先做知识协作试点，再开放有限自动协作及独立高级能力；
- 批准试点每席位单 Channel、独立数据/凭据/受限运行环境，以及产品 owner-only 与基础设施运维权限的区别；
- 批准优先验证公开运行时 Hook 的确定性准入，保留原生 Channel，明确前置读取隔离及故障拒绝；
- 批准补齐路由分支、自动邀请预算与显式多专家目标的区别；
- 批准知识目标受众授权、版本引用、缓存/会话撤回与已披露内容边界；
- 批准至少 30 个真实任务的评估方法，指标尚未测量；
- 本次仅更新文档。代码、部署、真实评估与发布需独立授权；不把函数级实验标为端到端验证。已有 HTML 设计册编辑不纳入本次修订，其更新范围由后续任务处理。
